...
// ClaudeCodeResponse represents the JSON response from Claude CLI
...
func main() {
	if len(os.Args) < 2 {
		...
		fmt.Fprintf(os.Stderr, "  health    - Check if claude CLI is available\n")
		os.Exit(1)
	}
	...
	case "stream":
		fmt.Fprintf(os.Stderr, "Streaming not supported by Claude Code CLI\n")
		os.Exit(1)
	case "health":
...
func handleGenerate() error {
	...
	// Build command: claude --print --output-format json --model <model> "<prompt>"
	args := []string{
		"--print",
		"--output-format", "json",
		"--model", model,
	...
	if err := json.Unmarshal(output, &claudeResp); err != nil {
		...
		claudeResp.Text = string(output)
	}

	// Convert to our response format
	resp := GenerateResponse{
		Content:      claudeResp.Text,
		TokensUsed:   0, // Claude CLI doesn't report token usage in --print mode
		InputTokens:  0,
		OutputTokens: 0,
		Model:        model,
