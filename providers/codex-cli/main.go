...
func main() {
	...
	case "stream":
		fmt.Fprintf(os.Stderr, "Streaming not supported by Codex CLI\n")
		os.Exit(1)
	case "health":
...
func handleGenerate() error {
	...
	// Get model from config (optional for codex)
	var model string
	...
	cmd, err := safeutil.SafeCommand(ctx, "codex", args...)
	if err != nil {
		return fmt.Errorf("failed to build codex-cli command: %w", err)
	}
	output, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("codex command failed: %w: %s", err, string(output))
	}

	// Convert to our response format
	resp := GenerateResponse{
		Content:      string(output),
		TokensUsed:   0, // Codex CLI doesn't report token usage in exec mode
		InputTokens:  0,
		OutputTokens: 0,
		Model:        model,
...
func handleHealth() error {
	...
	cmd, err := safeutil.SafeCommand(context.Background(), "codex", "--version")
	if err != nil {
		return fmt.Errorf("failed to build codex-cli health command: %w", err)
	}
