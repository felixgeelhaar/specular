...
func LoadProvidersConfig(path string) (*ProvidersConfig, error) {
	...
	return LoadProvidersConfigFromBytes(data)
}

// LoadProvidersConfigFromBytes parses provider config data from bytes
func LoadProvidersConfigFromBytes(data []byte) (*ProvidersConfig, error) {
...
func ValidateProviderConfig(config *ProviderConfig) error {
	...
	case ProviderTypeCLI:
		// CLI providers must have a path
		if path, ok := config.Config["path"].(string); !ok || path == "" {
			return fmt.Errorf("CLI providers require 'path' in config")
		}
...
func DefaultProvidersConfig() *ProvidersConfig {
	return &ProvidersConfig{
		Providers: defaultProvidersFromDescriptors(),
...
func defaultProvidersFromDescriptors() []ProviderConfig {
	providers := make([]ProviderConfig, 0, len(Descriptors()))
	for _, desc := range Descriptors() {
		agg := NewProviderAggregate(desc)
		providers = append(providers, agg.Config)
	}
	return providers
}

// ApplyRecommendedProviders enables the requested providers on the config and disables others.
func ApplyRecommendedProviders(config *ProvidersConfig, recommended []string) {
	if config == nil {
		return
	}
	...
	for i := range config.Providers {
		if _, ok := recSet[config.Providers[i].Name]; ok {
			config.Providers[i].Enabled = true
			enabledCount++
		} else {
			config.Providers[i].Enabled = false
		}
	}

	if enabledCount == 0 && len(config.Providers) > 0 {
		config.Providers[0].Enabled = true
	}
}

// ConfigFromRecommended returns a providers configuration with the supplied recommended providers enabled.
func ConfigFromRecommended(recommended []string) *ProvidersConfig {
	config := DefaultProvidersConfig()
	if len(recommended) > 0 {
		ApplyRecommendedProviders(config, recommended)
	}
	return config
}
...
func SaveProvidersConfigExample(config *ProvidersConfig, path string) error {
	...
	builder.WriteString("# Example provider configuration generated by specular\n")
	builder.WriteString("# Copy or reference this file to seed your own .specular/providers.yaml\n\n")
...
func generateProviderConfig(providerName string) *ProviderConfig {
	switch providerName {
	case "ollama":
		// Native provider uses built-in plugin via Ollama HTTP API
