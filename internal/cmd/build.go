...
func init() {
	...
	// build run flags
	buildRunCmd.Flags().String("plan", "plan.json", "Plan file to execute")
	buildRunCmd.Flags().String("policy", ".specular/policy.yaml", "Policy file for enforcement")
	buildRunCmd.Flags().Bool("dry-run", false, "Show what would be executed without running")
	buildRunCmd.Flags().String("manifest-dir", ".specular/runs", "Directory for run manifests")
	...
	buildRunCmd.Flags().String("checkpoint-dir", ".specular/checkpoints", "Directory for checkpoints")
	...
	buildRunCmd.Flags().Bool("enable-cache", true, "Enable Docker image caching")
	buildRunCmd.Flags().String("cache-dir", ".specular/cache", "Directory for image cache")
	buildRunCmd.Flags().Duration("cache-max-age", 7*24*time.Hour, "Maximum cache age")
